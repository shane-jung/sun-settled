import { getSubscriptionPlan } from "@/lib/fetchData"
import { Garden, GardenWithSubscribers, Subscriber } from "@/types"
...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  } = await getSubscriptionPlan({ id: params.id })

  const subscribers: Subscriber[] = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/subscribers?planId=${params.id}`
  ).then((res) => res.json())

  const gardens: GardenWithSubscribers[] = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/gardens?include=subscribers`
  ).then((res) => res.json())

  const subscribersWithPlan = gardens.map((garden: GardenWithSubscribers) => {
